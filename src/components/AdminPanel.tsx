...
import { Movie, Episode, AppSettings, StoryItem, BannerItem, SeasonInfo, AdultBannerItem } from '../types';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [adultItems, setAdultItems] = useState<AdultBannerItem[]>([]);
  const [adultTitle, setAdultTitle] = useState('');
  const [adultThumb, setAdultThumb] = useState('');
  const [adultLink, setAdultLink] = useState('');
  const [adultCategory, setAdultCategory] = useState('');
  ...
  const [maintenanceMessage, setMaintenanceMessage] = useState('আমাদের সার্ভার আপডেট চলছে। শীঘ্রই ফিরে আসছি!');
  const [maintenanceStartTime, setMaintenanceStartTime] = useState('');
  const [maintenanceEndTime, setMaintenanceEndTime] = useState('');
  const [maintenanceButtonText, setMaintenanceButtonText] = useState('আমাদের Channel এ Join করুন');
  const [maintenanceButtonLink, setMaintenanceButtonLink] = useState('');
  // ✅ Task System
  ...
  const applySettingsData = (data: any) => {
    ...
    setMaintenanceMessage(data.maintenanceMessage || 'আমাদের সার্ভার আপডেট চলছে। শীঘ্রই ফিরে আসছি!');
    setMaintenanceStartTime(data.maintenanceStartTime || '');
    setMaintenanceEndTime(data.maintenanceEndTime || '');
    setMaintenanceButtonText(data.maintenanceButtonText || 'আমাদের Channel এ Join করুন');
    setMaintenanceButtonLink(data.maintenanceButtonLink || '');
  ...
  // ══════════════════════════════════════════
  // 18+ CONTENT FUNCTIONS
  // ══════════════════════════════════════════

  const fetchAdult = async () => {
    try {
      const snap = await getDocs(query(collection(db, 'adultContent'), orderBy('order', 'asc')));
      const items = snap.docs.map(d => ({ id: d.id, ...d.data() })) as AdultBannerItem[];
      setAdultItems(items);
    } catch(e) { console.warn('fetchAdult error:', e); }
  };

  const handleAddAdult = async () => {
    if (!adultTitle.trim() || !adultThumb.trim() || !adultLink.trim()) {
      alert('Title, Thumbnail URL আর Watch Link সব দিতে হবে!');
      return;
    }
    setAdultLoading(true);
    try {
      await addDoc(collection(db, 'adultContent'), {
        title: adultTitle.trim(),
        thumbnail: adultThumb.trim(),
        channelLink: adultLink.trim(),
        category: adultCategory.trim() || 'General',
        order: adultItems.length + 1,
        isActive: true,
        createdAt: serverTimestamp(),
      });
      setAdultTitle('');
      ...
      setAdultLink('');
      setAdultCategory('');
      showSuccess('✅ 18+ Content যোগ হয়েছে!');
      fetchAdult();
    } catch(e) {
      console.warn('handleAddAdult error:', e);
      alert('Error! Firestore rules চেক করো।');
    }
    setAdultLoading(false);
  };

  const handleDeleteAdult = async (id: string) => {
    if (!confirm('এই content delete করবে?')) return;
    try {
      await deleteDoc(doc(db, 'adultContent', id));
      showSuccess('🗑️ Deleted!');
      fetchAdult();
    } catch(e) { console.warn(e); }
  };

  const handleToggleAdult = async (id: string, current: boolean) => {
    try {
      await updateDoc(doc(db, 'adultContent', id), { isActive: !current });
      fetchAdult();
    } catch(e) { console.warn(e); }
  };
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const settingsData = {
        ...
        maintenanceButtonLink: maintenanceButtonLink,
    ...
    <div className="fixed inset-0 bg-[#0d0d10] z-50 overflow-hidden flex flex-col">
      ...
      <div className="bg-[#0d0d10] border-b border-white/5 px-4 flex gap-1 overflow-x-auto no-scrollbar py-2">
        {[
          ...
          { id: 'adult', icon: ImageIcon, label: '18+ Content' },
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="max-w-6xl mx-auto">
          ...
          {activeTab === 'adult' && (
            <div className="space-y-6">
              <h2 className="text-lg font-black text-white tracking-tight">🔞 18+ Content Management</h2>

              {/* Add new */}
              <div className="bg-[#18181b] rounded-2xl p-4 space-y-3 border border-white/5">
                <h3 className="text-sm font-bold text-pink-400">নতুন 18+ Content যোগ করো</h3>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={adultTitle}
                    onChange={e => setAdultTitle(e.target.value)}
                    placeholder="Content এর নাম..."
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                </div>

                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Thumbnail URL * (YouTube 16:9 size)</label>
                  <input
                    type="text"
                    value={adultThumb}
                    onChange={e => setAdultThumb(e.target.value)}
                    placeholder="https://... (image link)"
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                  {adultThumb && (
                    <img src={adultThumb} className="mt-2 w-full max-w-[220px] aspect-video object-cover rounded-lg border border-white/10" />
                  )}
                </div>

                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Watch Link * (Watch Now button এ যাবে)</label>
                  <input
                    type="text"
                    value={adultLink}
                    onChange={e => setAdultLink(e.target.value)}
                    placeholder="https://t.me/... বা যেকোনো link"
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                </div>

                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Category (যেমন: Videos, Series, Photos)</label>
                  <input
                    type="text"
                    value={adultCategory}
                    onChange={e => setAdultCategory(e.target.value)}
                    placeholder="General / Videos / Series..."
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                </div>
                ...
                <button
                  ...
                  style={{ background: adultLoading ? 'rgba(236,72,153,0.3)' : 'linear-gradient(135deg, #ec4899, #8b5cf6)' }}
                >
                  {adultLoading ? 'যোগ হচ্ছে...' : '+ যোগ করো'}
                </button>
              </div>

              {/* Current list */}
              <div>
                <h3 className="text-sm font-bold text-white mb-3">Current 18+ Content ({adultItems.length})</h3>
                {adultItems.length === 0 ? (
                  <p className="text-zinc-500 text-sm text-center py-8">কোনো content নেই</p>
                ) : (
                  <div className="space-y-3">
                    {adultItems.map(item => (
                      <div key={item.id} className="bg-[#18181b] rounded-xl p-3 border border-white/5 flex gap-3">
                        <img src={item.thumbnail} className="w-24 aspect-video object-cover rounded-lg shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-white truncate">{item.title}</p>
                          <p className="text-xs text-zinc-500 truncate mt-0.5">{item.channelLink}</p>
                          <div className="flex gap-2 mt-2">
                            <button
                              onClick={() => handleToggleAdult(item.id, item.isActive)}
                              className={`text-xs px-2.5 py-1 rounded-lg font-bold ${item.isActive ? 'bg-green-500/20 text-green-400' : 'bg-white/5 text-zinc-500'}`}
                            >
                              {item.isActive ? 'Active' : 'Hidden'}
                            </button>
                            <button
                              onClick={() => handleDeleteAdult(item.id)}
                              className="text-xs px-2.5 py-1 rounded-lg bg-red-500/20 text-red-400 font-bold"
                            >
                              Delete
                            </button>
                          </div>
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              {/* ── MAINTENANCE MODE ── */}
              <div className="bg-red-500/8 border border-red-500/20 rounded-[20px] p-5 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">🚧</span>
                    <div>
                      <h3 className="text-red-400 font-black text-base">Maintenance Mode</h3>
                      <p className="text-zinc-500 text-xs">চালু করলে app এ maintenance screen দেখাবে</p>
                    </div>
                  </div>
                  <button onClick={() => setMaintenanceEnabled(!maintenanceEnabled)}
                    className={`relative w-14 h-7 rounded-full transition-all ${maintenanceEnabled ? 'bg-red-500' : 'bg-white/10'}`}>
                    <div className={`absolute top-0.5 w-6 h-6 bg-white rounded-full shadow transition-all ${maintenanceEnabled ? 'left-7' : 'left-0.5'}`} />
                  </button>
                </div>
                {maintenanceEnabled && (
                  <div className="bg-red-500/15 border border-red-500/25 rounded-xl p-3 flex items-center gap-2">
                    <span className="text-red-400 text-sm font-black">⚠️ এখন সব user maintenance screen দেখছে!</span>
                  </div>
                )}
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-zinc-500 mb-1.5 block">Message (user কে কী দেখাবে)</label>
                    <textarea value={maintenanceMessage} onChange={e => setMaintenanceMessage(e.target.value)} rows={2}
                      className="w-full px-4 py-3 bg-[#0d0d10] border border-white/8 rounded-[14px] text-white text-sm outline-none focus:border-red-500/30 resize-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-zinc-500 mb-1.5 block">শুরু সময়</label>
                      <input value={maintenanceStartTime} onChange={e => setMaintenanceStartTime(e.target.value)}
                        placeholder="যেমন: রাত ১০টা"
                        className="w-full px-3 py-2.5 bg-[#0d0d10] border border-white/8 rounded-[12px] text-white text-sm outline-none" />
                    </div>
                    <div>
                      <label className="text-xs text-zinc-500 mb-1.5 block">শেষ সময়</label>
                      <input value={maintenanceEndTime} onChange={e => setMaintenanceEndTime(e.target.value)}
                        placeholder="যেমন: সকাল ৬টা"
                        className="w-full px-3 py-2.5 bg-[#0d0d10] border border-white/8 rounded-[12px] text-white text-sm outline-none" />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs text-zinc-500 mb-1.5 block">Button এর লেখা</label>
                    <input value={maintenanceButtonText} onChange={e => setMaintenanceButtonText(e.target.value)}
                      placeholder="যেমন: আমাদের Channel এ Join করুন"
                      className="w-full px-4 py-3 bg-[#0d0d10] border border-white/8 rounded-[14px] text-white text-sm outline-none focus:border-red-500/30" />
                  </div>
                  <div>
                    <label className="text-xs text-zinc-500 mb-1.5 block">Button Link (Telegram Channel / Mini App)</label>
                    <input value={maintenanceButtonLink} onChange={e => setMaintenanceButtonLink(e.target.value)}
                      placeholder="https://t.me/yourchannel"
                      className="w-full px-4 py-3 bg-[#0d0d10] border border-white/8 rounded-[14px] text-white text-sm outline-none focus:border-red-500/30 font-mono" />
                    <p className="text-zinc-600 text-[10px] mt-1">Telegram link হলে in-app এ খুলবে। খালি রাখলে Channel link ব্যবহার হবে।</p>
                  </div>
                </div>
                <button onClick={handleSaveSettings} disabled={loading}
                  className={`w-full py-3 rounded-[14px] font-black text-sm flex items-center justify-center gap-2 transition-all ${maintenanceEnabled ? 'bg-red-500 text-white' : 'bg-white/10 text-zinc-400'}`}>
                  <Save size={15} />{maintenanceEnabled ? '🚧 Maintenance চালু রেখে Save করো' : '💾 Save করো'}
                </button>
              </div>
    ...
    match /{document=**} {
      ...
      allow write: if request.auth != null;
              ...
              <div className="bg-[#1a1a1d] rounded-[20px] p-5 space-y-4 border border-amber-500/20">
                ...
                {/* Ad Cooldown — সব Ad এ apply হয় */}
                <div className="bg-[#0d0d10] rounded-[16px] p-4 border border-blue-500/20 space-y-3 mt-2">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-lg">⏱️</span>
                    <div>
                      <p className="text-blue-300 text-sm font-black">Ad Cooldown</p>
                      <p className="text-zinc-600 text-[10px]">প্রতিটা Ad শেষে কত সেকেন্ড wait — Movie, Episode ও Tasks সব এ apply হবে</p>
                    </div>
                  </div>
                  ...
                  <p className="text-zinc-600 text-[10px]">0 = কোনো wait নেই। Recommended: 5</p>
                </div>
